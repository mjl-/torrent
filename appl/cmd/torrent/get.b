...
init(nil: ref Draw->Context, args: list of string)
{
	...
	# xxx
	#peers = array[] of {("vanheerde.net", 12208, array of byte "T03H-----eeuYHUFifYr")};
...
dopeer(ip: string, port: int, peerid: array of byte): string
{
	...
	msg := <-inmsg =>
		...
		Notinterested =>
			...
			for(j = 0; j < len m.d; j++)
				say(sprint("0x%02x", int m.d[j]));
			say("assuming it has all pieces..."); # xxx
			outmsg <-= ref Msg.Interested();
                Piece =>
			say(sprint("remote sent data for piece=%d begin=%d length=%d", m.index, m.begin, len m.d));
			if(m.begin != begin || len m.d != length)
				fail(sprint("remote sent bad begin (have %d, want %d) or length (%d, %d)", m.begin, begin, len m.d, length));
			piece.d[m.begin:] = m.d;
